...
def test_change_head_citizen_type() -> None:
    s = society_from_file_demo()
    s.change_citizen_type(1)
    assert s.get_head().cid == 1
    assert isinstance(s.get_head(), Citizen)
    s.change_citizen_type(1, 'Finance')
    assert s.get_head().cid == 1
    assert isinstance(s.get_head(), DistrictLeader)
    assert s.get_head().get_district_name() == 'Finance'
...
if __name__ == '__main__':
    ...
    pytest.main(['a2_sample_test_james.py'])
