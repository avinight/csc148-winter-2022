...
class Citizen:
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        for i in range(1, len(lst)):
            if lst[i - 1] > lst[i]:
                lst = merge(lst[:i], lst[i:])

        for subordinate in lst:
    ...
    def get_district_name(self) -> str:
        ...
        """
        # Note: This method must call itself recursively
        if self.get_superior():
            c = self.get_superior().get_district_name()
            return c
        return ''
    ...
    def rename_district(self, district_name: str) -> None:
        ...
        """
        # Note: This method must call itself recursively
        c_sup = self.get_superior()
        if c_sup:
            return c_sup.rename_district(district_name)
        return None
    ...
    def get_highest_rated_subordinate(self) -> Citizen:
        ...
        m = self.get_direct_subordinates()[0]
        for sub in self.get_direct_subordinates():
            if sub.rating > m.rating:
                m = sub
        return m
...
class Society:
    ...
    def add_citizen(self, citizen: Citizen, superior_id: int = None) -> None:
        ...
        elif self._head and superior_id:
            self.get_citizen(superior_id).add_subordinate(citizen)
        else:
            # for sub in self.get_head().get_direct_subordinates():
            #     self.get_head().remove_subordinate(sub.cid)
            # for sub in citizen.get_direct_subordinates():
            #     citizen.remove_subordinate(sub.cid)
            citizen.add_subordinate(self.get_head())
            self.set_head(citizen)
    ...
    def change_citizen_type(self, cid: int,
        ...
        """
        c1 = self.get_citizen(cid)
        c1_sub = c1.get_direct_subordinates()
        c1_sup = c1.get_superior()
        c1_mod = None
        if isinstance(c1, DistrictLeader):
            c1_mod = Citizen(
                c1.cid,
                c1.manufacturer,
                c1.model_year,
        ...
        elif isinstance(c1, Citizen):
            c1_mod = DistrictLeader(
                c1.cid,
                c1.manufacturer,
                c1.model_year,
                c1.job,
        ...
        c1_mod.set_superior(c1_sup)
        if c1_sup:
            c1_sup.remove_subordinate(c1.cid)
            c1_mod.get_superior().add_subordinate(c1_mod)

        for sub in c1_sub:
            c1.remove_subordinate(sub.cid)
            c1_mod.add_subordinate(sub)

        if self.get_head() == c1:
            self.set_head(c1_mod)
        return c1_mod
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        c = citizen
        c_sup = c.get_superior()

        c.cid, c_sup.cid = c_sup.cid, c.cid
        c.rating, c_sup.rating = c_sup.rating, c.rating
        c.manufacturer, c_sup.manufacturer = c_sup.manufacturer, c.manufacturer
        c.model_year, c_sup.model_year = c_sup.model_year, c.model_year

        # c.job, c_sup.job = c_sup.job, c.job
        #
        # if isinstance(c_sup, DistrictLeader):
        #     c = self.change_citizen_type(c.cid, c_sup.get_district_name())
        #     c_sup = self.change_citizen_type(c_sup.cid)
        #
        # c_sup.remove_subordinate(c.cid)
        # c.become_subordinate_to(c_sup.get_superior())
        # if c_sup.get_superior():
        #     c_sup.get_superior().remove_subordinate(c_sup.cid)
        #
        # c_sup_sub = c_sup.get_direct_subordinates()
        # c_sub = c.get_direct_subordinates()
        #
        # if c_sup_sub:
        #     for sub in c_sup_sub:
        #         c.add_subordinate(sub)
        #         c_sup.remove_subordinate(sub.cid)
        # elif c_sub:
        #     for sub in c_sub:
        #         c_sup.add_subordinate(sub)
        #         c.remove_subordinate(sub.cid)
        #
        # c_sup.become_subordinate_to(c)
    ...
    def promote_citizen(self, cid: int) -> None:
        ...
        c_sup = c.get_superior()
        while c_sup and (c_sup.rating < c.rating
                         and not isinstance(c, DistrictLeader)):
            c = self._swap_up(c)
            lst = c.get_direct_subordinates()
            for i in range(1, len(lst)):
                if lst[i - 1] > lst[i]:
                    c.remove_subordinate(lst[i].cid)
                    c.add_subordinate(lst[i])
            c_sup = c.get_superior()
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        if self._head == c:
            if c_sub:
                c_sup = c.get_highest_rated_subordinate()
                self.set_head(c.get_highest_rated_subordinate())
            else:
                self._head = None
        ...
        for sub in c_sub:
            ...
            if sub != self._head:
                c_sup.add_subordinate(sub)
                sub.set_superior(c_sup)

        if c_sup:
            c_sup.remove_subordinate(cid)
...
class DistrictLeader(Citizen):
    ...
    def get_district_citizens(self) -> List[Citizen]:
        ...
        """
        lst = [self]
        lst.extend(self.get_all_subordinates())  # Possible test case with
        # extend
        return lst
