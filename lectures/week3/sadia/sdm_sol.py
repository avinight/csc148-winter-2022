...
class SuperDuperManager:
    ...
    === Sample Usage ===  # Answer for Q3 in handout
    >>> sdm = SuperDuperManager()
    >>> sdm.add_vehicle('Car', '007', 100)
    >>> sdm.get_vehicle_fuel('007')
    100
    >>> sdm.move_vehicle('007', 5, 10)
    >>> sdm.get_vehicle_position('007')
    (5, 10)
    """
    # === Private Attributes ===
    # _vehicles:
...
class Car(Vehicle):
    ...
    def __init__(self, fuel: int) -> None:
        ...
        # self.position = (0, 0) --- don't do this; this duplicates code already in Vehicle's init
...
if __name__ == '__main__':
    ...
    doctest.testmod()
    import python_ta
    python_ta.check_all(config={'extra-imports': ['math', 'random']})
