...
=== Module Description ===
We came back to the general Tree class today, Wednesday April 6th, to write
the method to_tree.  We wrote the method, but need to think further about
invalid inputs.  We currently catch one (a simple int is not a valid
representation of a tree according to the worksheet we were using). But are
there others?

In class, I added a precondition:
    obj is a valid representation of a tree, as per the handout.
But this was not the right way to handle invalid inputs -- the docstring tells
us that we have to notice them and return None.  So I have modified the
docstring below.

I've also added a doctest.  Does our code pass it?  What doctests could we
add to check the various possible invalid inputs?
"""
...
class Tree:
    ...
    def delete_item(self, item: Any) -> bool:
        ...
        if self.is_empty():
            ...
            # delete.
            for subtree in self._subtrees:
                # Hey subtree, you're a Tree. Delete this item if you can,
                # and report back.
                successful = subtree.delete_item(item)
                if successful:
                    # Know: my child successfully deleted the item. I'm going
                    # to report back True, but first:
                    # if an empty tree was left there, I need to get rid of it.
                    if subtree.is_empty():
                        # Remove subtree from self._subtrees.
                        self._subtrees.remove(subtree)
                        # Normally we would NEVER EVER mutate a list that we
                        # are iterating over. It's only okay because we will
                        # IMMEDIATELY return.
                    return True
            return False
...
def to_tree(obj: Union[int, list]) -> Optional[Tree]:
    ...
    Return None if <obj> is not a valid representation of a tree, as defined
    in the handout, i.e., a nested list, where the first sub-nested-list is the
    root of the tree, and every other sub-nested-list is the nested list
    representation of one of the tree's subtrees.

    The nested list representation of an empty tree is simply the empty list.
    The nested list representation of a tree with a single item x is [x].
    ...
    if isinstance(obj, int):
        return None  # INVALID representation of a tree
    elif obj == []:  # Pycharm wants us to say "if not obj" but I prefer this.
        # Return an empty tree.
        return Tree(None, [])
    else:
        # Try to handle case (b) at the same time as the big case (d).
        # In fact, we did.
        new_root = obj[0]
        new_subtrees = []
        # Make a tree from each subnested list after the one that represented
        # the root.  Put them all in a list of subtrees for the root.
        for sublist in obj[1:]:
            subtree = to_tree(sublist)
            new_subtrees.append(subtree)
        # We know the root and all its subtrees.  Make a tree out of that.
