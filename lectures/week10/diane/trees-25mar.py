...
=== Module Description ===
Tree code as of Friday, March 25.  Today we came back to revisit deletion on
a general tree to address a problem: If a child successfully deletes the item,
the child could have ended up being an empty tree -- and we're left with that
empty tree in our list of subtrees.  These will accumulate if we keep doing
deletes and never clean up.

We modified the code to check for and remove the empty tree in this situation.
"""
