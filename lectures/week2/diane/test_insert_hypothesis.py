"""Testing: an example of property-based testing with the hypothesis module
...
=== Module description ===
This module contains a few simple hypothesis tests for insert_after.
...
all of the Python requirements for the course, including hypothesis.
"""
...
def test_new_item_count(lst: List[int], n1: int, n2: int) -> None:
    ...
    """
    # Ie., we should check that length of the list has increased? No,
    # it could stay the same.
    # But we DO know that the length should go up by: the number
    # of occurrences of n1.
    occurrences = lst.count(n1)
    original_length = len(lst)
    insert_after(lst, n1, n2)
    assert len(lst) == original_length + occurrences
